...
export const listQuerySchema = z.object({
  alignment: z.enum(['hero', 'villain']).optional(),
  page: z.coerce.number().int().min(1, { message: 'Page deve ser maior que 0' }).default(1),
  limit: z.coerce.number().int().min(1, { message: 'Limit deve ser maior que 0' }).max(50, { message: 'Limit deve ser no máximo 50' }).default(10),
});
...
export const battleQuerySchema = z.object({
  hero1: z.coerce.number().int().positive({ message: 'Hero1 deve ser positivo' }),
  hero2: z.coerce.number().int().positive({ message: 'Hero2 deve ser positivo' }),
